...
import { issueBookAction } from '../actions/libraryActions'
import { useDispatch, useSelector } from 'react-redux'
import { toast } from 'react-toastify'
import 'react-toastify/dist/ReactToastify.css';

function LibraryIssueBook({ toastPropertyProps }) {
    ...
    const issueBookHandler = async (e) => {
        ...
        if (Object.keys(bookData).length !== 0 && (Object.keys(stdLibraryData).length !== 0 || isStdDataShow)) {
            ...
            if (bookData.available === 0) {
                ...
                // checking whether student have issued 3 Books or not
                if (stdLibraryData?.books?.length === 3) {
                    toast.error('Student already issued 3 books!', toastPropertyProps)
                } else {
                    ...
                    if (stdLibraryData?.books?.map(book => book.bookNo === bookNo).includes(true)) {
                        toast.info('Student Already issued this book', toastPropertyProps)
                    } else {
                        try {
                            dispatch(issueBookAction(bookNo, studentId, category))
                            toast.success('Book Issued Successfully!!', toastPropertyProps);
                        } catch (error) {
                            toast.error('Something Went Wrong!!', toastPropertyProps);
                        }
    ...
    return (
        <div className='d-flex flex-column justify-content-center align-items-center w-100'>
            ...
            <Form className='mt-3 w-75' onSubmit={issueBookHandler}>
